...
import { BookOpen, Bot, ChevronLeft, FileText, History, LogOut, Menu, MessageSquarePlus, Settings, ShieldCheck, UserRound } from "lucide-react";
...
type Page = "chat" | "history" | "documents" | "settings";
const items: { id: Page; label: string; icon: typeof History; adminOnly?: boolean }[] = [
...
export default function App() {
  ...
  const navigate = (target: Page) => {
    if (target === "documents" && user.role !== "admin") return;
    setPage(target);
    setMobileOpen(false);
  };
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${collapsed ? "collapsed" : ""} ${mobileOpen ? "mobile-open" : ""}`}>
      ...
      <nav aria-label="Application"><p className="nav-label">WORKSPACE</p>{items.filter(x => !x.adminOnly || user.role === "admin").map(({ id, label, icon: Icon }) => <button key={id} className={`nav-item ${page === id ? "active" : ""}`} onClick={() => navigate(id)}><Icon size={18}/><span>{label}</span></button>)}</nav>
      <div className="sidebar-footer"><div className="secure-note"><ShieldCheck size={16}/><span>Secure workspace</span></div><button className="profile-card" onClick={() => navigate("settings")}><Avatar user={user}/><span><b>{user.full_name}</b><small>{user.role === "admin" ? "Administrator" : user.role.charAt(0).toUpperCase() + user.role.slice(1)}</small></span></button></div>
    </aside>
    <main className="main"><header className="header"><button className="icon-button mobile-menu" onClick={() => setMobileOpen(true)} aria-label="Open navigation"><Menu/></button><div className="header-title"><span className="eyebrow">ENTERPRISE KNOWLEDGE INTELLIGENCE</span><strong>{page === "chat" ? "Knowledge workspace" : items.find(x => x.id === page)?.label}</strong></div><div className="profile-menu"><span className="presence"/><Avatar user={user}/><div><b>{user.full_name}</b><small>{user.role}</small></div><button className="logout" onClick={logout} aria-label="Log out"><LogOut size={17}/></button></div></header>
      <AnimatePresence mode="wait"><motion.section key={page} className="page" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: .2 }} style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column", overflowX: "hidden", overflowY: "scroll"}}>
        {page === "chat" && <ChatPage user={user}/>} {page === "history" && <HistoryPage onOpen={() => { navigate("chat"); }} />} {page === "documents" && user.role === "admin" && <DocumentsPage/>} {page === "settings" && <SettingsPage user={user}/>}</motion.section></AnimatePresence>
...
function SettingsPage({ user }: { user: User }) { return <div className="settings-page"><div><span className="eyebrow">ACCOUNT</span><h1>Settings</h1><p>Manage the preferences this workspace can support locally.</p></div><div className="settings-card"><UserRound/><div><b>{user.full_name}</b><p>{user.email} · {user.role === "admin"
  ? "Administrator"
  : user.role.charAt(0).toUpperCase() + user.role.slice(1)}</p></div></div><div className="settings-card"><Settings/><div><b>Retrieval experience</b><p>Choose a retrieval mode for each query in the composer. Technical diagnostics remain available per completed response.</p></div></div></div> }
